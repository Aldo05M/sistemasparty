...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('[data-gallery]').forEach(gallery => {
        ...
        const cards = Array.from(track.querySelectorAll('.triptych-item'));
        ...
        track.addEventListener('pointermove', event => {
            if (drag) track.scrollLeft = drag.scroll - (event.clientX - drag.x);
        });
        function endDrag() { drag = null; track.classList.remove('dragging'); }
        track.addEventListener('pointerup', endDrag);
        track.addEventListener('pointercancel', endDrag);
        track.addEventListener('lostpointercapture', endDrag);
        cards.forEach((card, index) => card.style.setProperty('--gallery-delay', `${index % 3 * 90}ms`));
        updateControls();
    });
    const revealTargets = document.querySelectorAll('[data-gallery-reveal]');
...
function updateCountdown() {
    // Ajusta aquí la fecha de tu próximo evento (24 Septiembre 2026 21:00 local)
    const eventDate = new Date('2026-09-24T21:00:00').getTime();
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Only initialize song request features if the page contains the song UI
    if (addSongBtn || songListEl) {
        if (addSongBtn) addSongBtn.addEventListener('click', addSong);
        // Autocompletado solo si existe el input
        if (document.getElementById('songName')) initAutocompletado();

        // Fetch initial songs only if supabaseClient is available
        if (typeof supabaseClient !== 'undefined') {
            fetchSongs();
            setInterval(fetchSongs, 5000);
        } else {
            // Mostrar estado si existe el contenedor de estado
            showStatus('La funcionalidad de solicitudes de canción está deshabilitada.', true);
        }
    }
});
